...
- The version has one source, `pyproject.toml`. release-please updates `__init__.py` itself
  (Python strategy), and `extra-files` cover the chart's `appVersion` (annotation) and the root
  package entry of `uv.lock` (jsonpath), so a release commit leaves `uv sync` clean.
